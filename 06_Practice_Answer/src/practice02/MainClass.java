...
public class MainClass {
	...
	public static void ex08() { 
		Scanner sc = new Scanner(System.in); // 스캐너로 몇자리 수를 만들지. 스캐너 선언. 
		System.out.print("몇 자리의 인증번호를 생성할까요? >>> ");
		int count = sc.nextInt(); //받은 값 저장 
		SecureRandom secureRandom = new SecureRandom(); // 인증번호 - 보안코드라 시큐어 랜덤 
		StringBuilder sbCode = new StringBuilder(); // 받은 코드를 하나로 합쳐야 함 
		for(int n = 0; n < count; n++) { // 반복문으로 받은 자리수만큼 반복을 해서 6자리 만큼 반복 시킴 
			double randNumber = secureRandom.nextDouble(); 
			if(randNumber < 0.33) { // 골고루 나와야 하므로 (대문자 소문자 숫자) 이프문으로 확률을 균등히 나오게 해야 함 
				sbCode.append((char)((int)(secureRandom.nextDouble() * 26) + 'A')); //한글자라 차타입 저장 가능, 알파벳 26자, 0~26 0일때 A 
			} else if(randNumber < 0.66) {
				sbCode.append((char)((int)(secureRandom.nextDouble() * 26) + 'a'));
			} else {
				sbCode.append((char)((int)(secureRandom.nextDouble() * 10) + '0')); //숫자는 총 10개 
			}
		}
		String code = sbCode.toString(); // - > 스트링 빌드를 변환 
	...
	public static void ex09() {
		...
		int totalDays = 0;
		// 년도를 totalDays에 누적
		for(int y = 1; y < year; y++) { //2022년까지만 더하겠다 
			totalDays += 365; //매해 무조건 더함 
			if((y % 4 == 0 && y % 100 != 0) || y % 400 == 0) { //조건에 해당하면 하루 추가 됨 
				totalDays++; 
			}
		}
		// 월을 totalDays에 누적
		int[] lastDays = {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30};
		//0번 인덱스에 0그대로 써줌, 인덱스대로 사용하기 위해. 
