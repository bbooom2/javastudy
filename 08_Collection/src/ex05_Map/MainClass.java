...
public class MainClass {
	...
	public static void ex01() {
		...
		/*
		 	...
		 	2. 용어 정리 
		 	 1) Entry : 하나의 데이터 (2개가 묶여 있는 게 Entry) 
		 	 2) Key : Entry의 구성 요소, 식별자 역할을 수행 (배열의 인덱스와 같은 역할) - '식별자역할' key
		 	 3) Value : Entry의 구성 요소, 실제 데이터(배열에 저장된 데이터 같은 역할)
		 	3. 주로 Key는 String을 사용한다. (변수 이름)  = Key, 변수이름 
		...
		// 추가 
		// put(Key, Value)
		person.put("name", "홍길동"); // 뒤에들어가는거 object 
		person.put("age", 30);
		
		// 수정
		// put(Key, Value)
		 person.put("age", 40); // 기존 Key와 동일한 Key를 전달하면 해당 Key 값의 Value가 수정된다. 
		 
		 //삭제 
		 //remove(Key) - 해당 키 값의 엔트리 지우는 것 
		 int age = (int)person.remove("age"); //삭제된 Value가 반환된다. Value는 오브젝트 타입이므로 캐스팅해야 사용할 수 있다 
	...
	public static void ex03() {
		...
		//1. Key만 모두 가져온 뒤, 해당 Key 값을 가진 Value를 가져오기 
